...
class Skyblock:
    ...
    def get_profile_skin_and_cape(self, uuid: str):
        api_request = requests.get(f"https://sessionserver.mojang.com/session/minecraft/profile/{uuid}")
        content = parse(api_request.content)
        return {"id": content["id"], "name": content["name"], "properties": content["properties"]}
